...
(pre-define
  ...
  sph-memory-status-group (convert-type "sph" uint8-t*)
  ...
  (sph-malloc-string size result) (sph-memory-malloc-string size (convert-type result uint8-t**))
...
(declare
  ...
  (sph-memory-status-name a) (uint8-t* status-t)
  (sph-memory-malloc size result) (status-t size-t void**)
  (sph-memory-malloc-string length result) (status-t size-t uint8-t**)
