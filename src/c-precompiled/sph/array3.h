...
#define array3_declare_type_custom(name, element_type, array3_alloc, array3_realloc, array3_free) \
  ...
  uint8_t name##_new(size_t size, name##_t* a) { \
    element_type* data; \
    data = array3_alloc((size * sizeof(element_type))); \
    if (!data) { \
      return (1); \
    }; \
    a->data = data; \
...
\
  ...
  uint8_t name##_resize(name##_t* a, size_t new_size) { \
    element_type* data = array3_realloc((a->data), (new_size * sizeof(element_type))); \
    if (!data) { \
      return (1); \
    }; \
    a->data = data; \
...
#define array3_get_pointer(a, index) (a.data + index)
...
#define array3_take(a, data, size, used) \
  ...
  a->size = size; \
  a->used = used
#define array3_last(a) (a.data)[(a.used - 1)]
#define array3_first_unused(a) (a.data)[a.used]
