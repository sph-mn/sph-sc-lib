...
(include-sc "../../sph/imht-set")
(define-macro test-element-count 10000000)

(define (test-zero set) (uint8_t imht-set-t*)
  (assert (= 0 (imht-set-find set 0))) (imht-set-add set 0)
  (assert (not (= 0 (imht-set-find set 0)))) (imht-set-remove set 0)
  (assert (= 0 (imht-set-find set 0))))

(define (insert-values set) (uint8_t imht-set-t*)
  (define counter size-t test-element-count)
  (while counter (imht-set-add set counter) (set counter (- counter 1))))
...
(define (main) int
  ...
  (test-zero set) (insert-values set)
  (test-value-existence set) (imht-set-destroy set) (printf "success\n") (return 0))
