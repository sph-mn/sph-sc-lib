...
#define local_define_malloc(variable_name,type,on_error) type* variable_name=malloc(sizeof(type));\
  if(!variable_name){on_error;}
#define local_memory_init(max_address_count) b0* sph_local_memory_addresses[max_address_count];\
